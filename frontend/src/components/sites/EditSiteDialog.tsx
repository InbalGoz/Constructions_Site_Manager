...
export function EditSiteDialog({
  ...
  // Load site data when dialog opens
  useEffect(() => {
    if (site && open) {
      setFormData({
        name: site.name || "",
        ...
        actualCost: site.actualCost ? String(site.actualCost) : "",
        imageUrl: site.imageUrl || "",
      });

      if (site.startDate) {
        setStartDate(new Date(site.startDate));
      }
      if (site.endDate) {
        setEndDate(new Date(site.endDate));
      }
    }
  }, [site, open]);
      ...
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader className="text-center pb-4">
          ...
          <DialogDescription className="text-base text-center">
            ערוך את פרטי האתר. כל השינויים יישמרו במערכת.
          </DialogDescription>
        </DialogHeader>
